...
var format = function(formComposants) {
    var existingKinds = {
        "input": input,
        "button": button
    };

    for (var i = 0; i < formComposants.length; i++) {
        if (existingKinds[formComposants[i].kind] != undefined) {
            formComposants[i] = existingKinds[formComposants[i].kind](formComposants[i]);
        }
        else {
            formComposants[i] = handleError("'" + formComposants[i].kind+ "' is not a form Composant kind.");
        }
    }
